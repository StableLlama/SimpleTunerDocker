...
# Fallback credentials if not passed as environment variables
...
# Create dynamic Caddyfile
{
  ...
  if [[ -n "${VAST_OPEN_TOKEN}" ]]; then
    ...
    echo "    # remember the session so the following requests are authorised."
    echo "    # NB: no 'uri query -token' here - that subdirective needs Caddy"
    echo "    #     >= 2.8 while Debian/Ubuntu ship 2.7. The root '/' case"
    echo "    #     above already redirects without the token; this route only"
    echo "    #     sees the token on the very first request of a deep link."
    echo "    @token_query {"
    echo "        query token=${VAST_OPEN_TOKEN}"
    echo "    }"
...
if [[ -v DIRECT_TRAINING ]]; then
  ...
  if [[ -v TRAINING_NAME ]]; then
    ...
    export GIT_TRAINING_TAG="${TRAINING_NAME}_${START_TIME}"
